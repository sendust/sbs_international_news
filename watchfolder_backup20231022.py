...
class encoder:
    ...
    def get_pipe(self):
        while self.get_pipe_continue:
            self.get_pipe_continue = psutil.pid_exists(self.pid)
        ...
        if self.checkoutput:
            if os.path.isfile(self.checkoutput):
                ...
                if size_outputfile:     # output check file exist and encoded output has a valid fie size
...
class scriptfinder:
    ...
    def get_script_title(self, infile):
        dict_get_script_title = {'reuter': self.get_script_title_reuter, 'aptn' : self.get_script_title_aptn, 'cnn' : self.get_script_title_cnn}
        try:
            updatelog(f'Probe script title ----\n{dict_get_script_title[self.mode](infile)}', True)   # Show few header lines..
        except Exception as e:
            updatelog(f'Error acquiring script title {e}')
        return dict_get_script_title[self.mode](infile)
    ...
    def get_script_title_reuter(self, infile):
    ...
    def get_script_title_aptn(self, infile):
    ...
    def get_script_title_cnn(self, infile):
        # BHDP_HE-117TU_FAUCI EXPECTS FIRST L_CNNA-ST1-20000000000571b1_175_0.mp4
        key = ""
        try:
...
class transfer:
    ...
    def remote_list(self, rpath="."):
        result = []
        try:
            f = ftplib.FTP(host = self.host, user = self.username, passwd = self.password, timeout = 8, encoding = 'utf-8')
            result = f.nlst(rpath)
            f.quit()
        except Exception as e:
...
def decode_protocol_sio(data):
    ...
    if ((data["protocol"] == "gui") and (data["engine"] == args.script)):
        ...
        if (req_data == "shutdown"):
        ...
        elif req_data.startswith("<get_ftplist>"):  # get ftp list (old version..
            connection = transfer(args.transfer, args.username, args.password)
            get_list = connection.remote_list()
            largedata = ""
            for each in get_list:
        ...
        elif (req_data.startswith("<get_incomelist>")):
            connection = transfer(args.transfer, args.username, args.password)
            get_list = connection.remote_list('/incoming')
            largedata = ""
            for each in get_list:
            ...
            data = client_req.split("><")[1][:-1]   # select second bracketted protocol
...
def write_xml_job(this):
    ...
    ET.SubElement(job, "Job_Src_ID").text = this.outputfiles["ID"]
    ...
    ET.SubElement(job, "Job_translation_Enable").text = ur.send_data["translation"]    # added 2023/8/6
...
updatelog(f'start watch folder....  python version is {sys.version}', True)
jobs = queue()
