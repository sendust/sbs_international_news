;  Modified 2019/7/8  - apply some modification on sendText()
; Modified 2020/4/20 - added, UDP REUSEADDR method
; Modified 2021/1/6   - Non-blocking mode (trial, change Variable Blocking True -> False)
; Modified 2021/1/8   - Remove Try catch on sendText()
...
class Socket
{
	...
	static Blocking := False, BlockSleep := 10
	...
	MsgSize()
	{
		...
		if (DllCall("Ws2_32\ioctlsocket", "UInt", this.Socket, "UInt", FIONREAD, "UInt*", argp) == -1)
		{
			...
			FileAppend, Error while get msge size, *
		}
		return argp
	}
	...
	SendText(Text, Flags:=0, Encoding:="UTF-8")					; Modified 2019/7/8 (try... catch)		(Remove try catch .. 2021/1/18)
	{
		VarSetCapacity(Buffer, StrPut(Text, Encoding) * ((Encoding="UTF-16"||Encoding="cp1200") ? 2 : 1))
		Length := StrPut(Text, &Buffer, Encoding)
		result := this.Send(&Buffer, Length - 1)
		return  result
	}
...
class SocketUDP extends Socket
{
	...
	SetReuseAddr(Enable)
	{
		...
		SetReuseUnicastPort(Enable)
	{
		static SOL_SOCKET := 0xFFFF, SO_BROADCAST := 0x20
		static SO_REUSEUNICASTPORT := 0x3007
		if (DllCall("Ws2_32\setsockopt"
			, "UInt", this.Socket ; SOCKET s
			, "Int", SOL_SOCKET   ; int    level
			, "Int", SO_REUSEUNICASTPORT  ; int    optname
			, "UInt*", !!Enable   ; *char  optval
			, "Int", 4) == -1)    ; int    optlen
			throw Exception("Error calling setsockopt",, this.GetLastError())
	}
