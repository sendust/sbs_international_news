...
class full_path_set:     # Accept file list and chkeck all files are exist
    ...
    def make_xml_job(self):
        ...
        ET.SubElement(job, "Job_Src_Path_HR").text = self.outputfiles["mxf"]
        ET.SubElement(job, "Job_Src_Path_LR").text = self.outputfiles["pxy"]
        ET.SubElement(job, "Job_Src_Path_CAT").text = self.outputfiles["jpg1"]
...
def do_encoding(filename):
    global path_done_report, folder_mxf, ur, jobs
    ...
    fp = full_path_set(folder_mxf, path_done_report)
    ...
    try:
        #proxy_folder = os.path.join(folder_mxf, outname)
...
def on_created(event):
    ...
    if get_extension(event.src_path).lower() == ".mp4":
        if (not get_name(event.src_path).startswith("WH16x9")) and (not args.polling):  # CNN exception
            do_encoding(event.src_path)
        else:
            updatelog("Ignore CNN exception.....", True)
...
def on_modified(event):
    ...
    if get_extension(event.src_path).lower() == ".mp4":
        if not get_name(event.src_path).startswith("WH16x9"):  # CNN exception
            do_encoding(event.src_path)
...
def on_moved(event):
    ...
    if get_extension(event.dest_path).lower() == ".mp4":
        if not get_name(event.dest_path).startswith("WH16x9"):  # CNN exception
            do_encoding(event.dest_path)
        else:
            updatelog("Ignore CNN proxy file...", True)
...
def show_tick_line(q):
    ...
    if not tqdm.disable_tick:
        print(f'Watch folder = {folder_watch}   <----->  {time.time()}  Wait incoming media...', end="\r")
...
class delete_old():
    ...
    def do_delete(self):
        ...
        for idx, path in enumerate(self.list_path):
            updatelog(f'Collect file list form path = {path}', True)
...
def decode_command(cso, client_data):
    if len(client_data):
        largedata = get_media_list(folder_watch)
...
def decode_command_new(address, client_req):
    ...
    if client_req.startswith("<get_income>"):
        largedata = get_media_list_ext(folder_watch, '*.mp4')
...
folder_watch = args.watchfolder
folder_mxf = args.target
timeout_encoder = args.timeout
ur = udp_reporter("127.0.0.1", args.port)

sc = scriptfinder(folder_watch, args.script)
...
ob.start(folder_watch)


path_done_report = os.path.join(os.path.dirname(__file__), 'done')

list_missing = gather_missing(path_done_report, folder_watch)

updatelog(f'Watch folder is "{folder_watch}" / target is "{folder_mxf}" / Encoder limit is {timeout_encoder} second', True)

deleteold = delete_old()
deleteold.add_path_age(path_done_report, 90)
deleteold.add_path_age(folder_mxf, 72)
deleteold.add_path_age(os.path.join(os.path.dirname(__file__), 'debug'), 168)
deleteold.do_delete()
deleteold.start_schedule(2)

set_report_data("tcpsvr", int(args.port + 1))
set_report_data("pid", os.getpid())
set_report_data("watch", folder_watch)
set_report_data("target", folder_mxf)
